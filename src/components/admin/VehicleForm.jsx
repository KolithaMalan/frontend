...
const VehicleForm = ({ vehicle, isOpen, onClose, onSuccess }) => {
  ...
  const validateForm = () => {
    ...
    } else if (!/^[A-Za-z]{2,3}-\d{4}$/.test(formData.vehicleNumber.trim())) {
      newErrors.vehicleNumber = 'Invalid format (e.g., AB-1234 or ABC-1234)';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const message = error.response?.data?.message || `Failed to ${isEditing ? 'update' : 'create'} vehicle`;
      toast.error(message);
    } finally {
      setLoading(false);
    }
